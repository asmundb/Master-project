...
read_SURFEX_RESULT <- function(res_path){
  ...
  exp_obs   <- array(dim=c(ndir))
  obs_out   <- array(dim=c(ndir))
#  Ho        <- array(dim=c(ndir,7,8))
  ...
  for (idir in 1:ndir){
    ...
    obs[idir] <- as.matrix(tmp2)
    time[idir] <- as.numeric(dtg)
    exp_obs[idir] <- tmp1[patch,nvar+1]
    obs_out[idir] <- tmp1[patch,nvar+2]
    for (v in 1:nvar){
      forecast[idir,v] <- tmp1[patch,v]
      analysis[idir,v] <- tmp1[patch,v+nvar+2]
      increment[idir,v]<- tmp1[patch,v+2*nvar+2]
    }
  }
  obs[which(obs > 900)] <- NA
