...
clay        <- ncvar_get(ncid, ncid$var$CLAY)
sand        <- ncvar_get(ncid, ncid$var$SAND)
nx <- ncid$dim$xx$len
ny <- ncid$dim$yy$len
nc_close(ncid)
...
loadHO <- function(path){
  nvar <- 7
  nobs <- 1
  files <- list.files(path, pattern="HO",recursive=T,full.names=T)
  nfiles <- length(files)
  
  H <- array(NA, dim=c(nvar,111,111,nfiles/nvar)) 
  K <- array(NA, dim=c(nvar,111,111,nfiles/nvar))
  
  ifile <- 1
  it <- 1
  while (ifile <= nfiles){
    for ( ivar in 1:nvar){
      tmp <- as.matrix(read.table(files[ifile]))
      tmp2 <- maskNreshape(tmp,frac_nature)
      H[ivar,,,it] <- tmp2[,,1]
      K[ivar,,,it] <- tmp2[,,2]
      ifile <- ifile + 1 
    }
