...
ln -sf /home/asmundb/SURFEX2/open_SURFEX_V8_0/src/HOMEMADE_ASSIM/assim.x ./soda.exe
...
# Run control + perturbed runs
  ...
  while [ $ens -le $nens ]; do
    ...
    then
      ...
      cp -f SURFOUT_${ens}.nc PREP_${ens}.nc
  ...
  do
	...
	then 
      ...
      while [ $ens -le $nens ]
      do
  	    mv -f SURFOUT_${ens}.nc  PREP_${ens}.nc # PREP_${aa}${mm}${jj}H${RR}_EKF_ENS${ens}.nc
    	ens=$(( $ens + 1 ))
      done
      
      if [ $RR -eq 06 ]
      then     #      OBSERVATIONS_161016H06.DAT
          cp -f INPUT/OBSERVATIONS_$aa$mm${jj}H${RR}.DAT OBSERVATIONS.DAT
      elif [ $RR -eq 18 ] 
      then
          cp -f INPUT/OBSERVATIONS_$aa$mm${jj}H${RR}.DAT OBSERVATIONS.DAT 
      else
          cp -f INPUT/SMOS_Default.DAT OBSERVATIONS.DAT 
      fi
...
#      mv -f increments_*.tar.gz OUTPUT/$EXP/
...
#      rm -f CANARI_NATURE_$aa0$mm0${jj0}H${RR0}.DAT
      rm -f OBSERVATIONS.DAT  

      #save output analysed files
      echo 'debug 3'
#      mv -f assim_$aa0$mm0${jj0}${NT}.tar.gz OUTPUT/$EXP/
      echo 'debug 4'
   ...
   #      mv -f prep_$aa0$mm0${jj0}${NT}.tar.gz /"/$EXP/
   #   fi
      echo 'debug 5'
      rm -f PREP_*.nc
